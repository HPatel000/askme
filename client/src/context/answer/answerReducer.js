...
export default (state, action) => {
  switch (action.type) {
    ...
    case ADD_ANSWER:
      state.answers[`${action.payload.question}`] = [
        action.payload,
        ...state.answers[`${action.payload.question}`],
      ]
      return {
    ...
    case UPDATE_ANSWER:
      state.answers[`${action.payload.question}`] = state.answers[
    ...
    case DELETE_ANSWER:
      state.answers[`${action.payload.queId}`] = state.answers[
        `${action.payload.queId}`
      ].filter(answer => answer._id !== action.payload.id)
      return {
        ...state,
        answers: state.answers,
        loading: false,
